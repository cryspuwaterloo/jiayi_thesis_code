...
class OneClassTouchAuth(Authenticator):
    ...
    def tps_to_feature_vector(tps, last_swipe_time):
        ...
        """
        fv = [0.0] * len(OneClassTouchAuth.FEATURE_LIST)
    ...
    def extract_features(self, raw_data, sel_activities=None):
        ...
        """
        tps = OneClassTouchAuth.raw_to_touchpoints(raw_data, sel_activities)
        # print(len(tps))
        ...
        while i < len(tps):
            ...
            if end_idx - start_idx > 5:
                # Cleanse data
                fv = OneClassTouchAuth.tps_to_feature_vector(
    ...
    def train_model(self, train_data, test_data=None):
        ...
        # for one-class classification, we drop the imposter set from training data
        train_data = [(y, x) for y, x in train_data if y == 0]
        ...
        n_classes = max(y_final) + 1

        if not self.clf:
            self.clf = [IsolationForest() for i in range(n_classes - 1)]

        # train model for each class
        X_by_class = [[] for i in range(n_classes)]
        for y, x in zip(y_final, X_final):
            X_by_class[y].append(x)

        thresholds = []  # threshold to be determined using cross validation
        for cl, xs in enumerate(X_by_class):
            if cl == 0:
                continue
            self.clf[cl - 1].fit(xs)
            scores = self.clf[cl - 1].decision_function(xs)
            thresholds.append(np.percentile(scores, 10))
    ...
    def train_model_with_eval(self, train_set, test_set):
        ...
        # resamp = imblearn.over_sampling.SMOTE(sampling_strategy='all')
        # dosamp = imblearn.under_sampling.CondensedNearestNeighbour()
        # resamp.fit_resample(X_train_new, y_train)
        X_final, y_final = X_train_new, y_train
        # print(Counter(y_train))
        # self.clf.fit(X_train_new, y_train)
        ...
        self.thresholds = thresholds
        pred = self._merge_predict(X_test_new)
    ...
    def authenticate(self, features):
        ...
        y_predict = self._merge_predict(X_train_new)
        return y_predict

    def _merge_predict(self, x, thresholds):
        scores = np.array([unary.decision_function(x) - thresholds[i]
                          for i, unary in enumerate(self.clf)])
        return compose_score_matrix(scores)

    def __init__(self, clf=None, scaler=None):
        self.clf = [] if clf is None else clf
        self.scaler = MinMaxScaler((0, 3)) if scaler is None else scaler
        self.threshold = []


def compose_score_matrix(arr):
    # Get the shape of the input array
    m, n = arr.shape

    # Initialize the output array with an extra row
    out = np.zeros((m+1, n))

    # Loop through each column
    for j in range(n):
        # Check if all elements in the column are negative
        if np.all(arr[:, j] <= 0):
            out[0, j] = 1
        else:
            # Add 0 to the top and set negative values to 0
            out[0, j] = 0
            out[1:, j] = arr[:, j]
            out[1:, j][out[1:, j] <= 0] = 0
